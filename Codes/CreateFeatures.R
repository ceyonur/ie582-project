...
Matches <- fread(paste0(RawDataPath,"matches.csv"))
Bets <- fread(paste0(RawDataPath,"bets.csv"))
Goals <- fread(paste0(RawDataPath,"goals.csv"))
Stats <- fread(paste0(RawDataPath,"stats.csv"))
Booking <- fread(paste0(RawDataPath,"Booking.csv"))
...
Matches[,`:=`(DateId = NULL, DateId_Season = NULL, Season = NULL,Outcome_forelo = NULL)]

Train_M_ids_1 <- sample(Matches[,M_id], size = nrow(Matches)*0.8, replace = FALSE, prob = NULL)
Train_M_ids_2 <- sample(Matches[,M_id], size = nrow(Matches)*0.8, replace = FALSE, prob = NULL)
Train_M_ids_3 <- sample(Matches[,M_id], size = nrow(Matches)*0.8, replace = FALSE, prob = NULL)

FeatureCols <- names(Matches)[6:ncol(Matches)]

save(Matches,Train_M_ids_1,Train_M_ids_2,Train_M_ids_3,FeatureCols, file = paste0(CleanDataPath,"TrainInput.rdata"))
