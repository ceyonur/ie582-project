---
title: "Project - Bookmaker Approach"
output:
  html_document:
    df_print: paged
---
## **Barış Işık**-**Ceyhun Onur**-**Seval Ata**
...
Note: bets.csv is removed from the repository since it's too big. You can put it in the same directory as "bet_extra.csv" file to run this code.
...
```{r}
unplayed_epl_matches <- matches[league_id == 148 & is.na(match_status) & epoch < as.integer(as.POSIXct("2020-01-03"))] 
...
We can find our submission RPS value by providing outcome set as follows. Do not run this code without changing the submission$outcome column accordingly to each match outcome.
